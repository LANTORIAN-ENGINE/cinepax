...
// Cette classification porte-t-elle un âge ?
//
// C'est la seule qui s'écrit avec un chiffre — « -12 ans », « -16 ans » — et
// c'est là que naît la confusion : le site emploie déjà « … ans » pour une
// tout autre chose, les tranches tarifaires (« 3 à 11 ans », « 12 à 24 ans »
// dans offers.age). Le même mot dit une fois qui peut entrer, une fois qui
// paie combien. Le lecteur n'a que la forme pour les distinguer : d'où la
// pastille rouge de la liste des films, qui dit « interdiction » avant même
// qu'on lise le chiffre.
export function ratingRestricts(raw) {
  const code = ratingCode(raw)
  return code?.key === 'under' || code?.key === 'a12'
}
...
// ─── Genres ───────────────────────────────────────────────────────────────────
// Le champ `Genre` de Veezi est saisi en anglais, quelle que soit la version du
// film : sur le catalogue relevé, trente-quatre écritures, dont « Action »,
// « Drama », « Horror » — affichées telles quelles jusqu'ici, y compris aux
// visiteurs francophones. Le jeu étant fermé et court, un dictionnaire suffit :
// pas de traduction automatique, donc pas d'appel réseau et un rendu stable.
//
// « Sci-Fi » et « Science Fiction » désignent le même genre et partagent leur
// clé. Une écriture inconnue est rendue telle quelle plutôt que masquée.
const GENRES = {
  ACTION:             'action',
  ADVENTURE:          'adventure',
...
// Libellé affichable d'un genre. `t` est la fonction de traduction du contexte
// i18n. Renvoie `null` quand le champ est vide.
export function genreLabel(raw, t) {
  if (raw == null || String(raw).trim() === '') return null
  const brut = String(raw).trim()
  const cle = GENRES[brut.toUpperCase().replace(/[^A-Z]/g, '')]
  return cle ? t(`genre.${cle}`) : brut
}
